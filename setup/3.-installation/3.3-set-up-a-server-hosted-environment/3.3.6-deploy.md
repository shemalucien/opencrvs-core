...
_KIBANA\_PASSWORD (A password for a Kibana user to login and monitor OpenCRVS stack health)_

_KIBANA\_USERNAME (A username for a Kibana user to login and monitor OpenCRVS stack health)_

_MONGODB\_ADMIN\_PASSWORD (Admin user database password)_

_MONGODB\_ADMIN\_USER (Admin user database username)_

MINIO\_ROOT\_PASSWORD _(A password for a Minio user to login to the Minio console to view supporting document attachments)_

MINIO\_ROOT\_USER _(A username for the Minio user)_

_DOCKER\_USERNAME - Your_ [_Dockerhub_](https://hub.docker.com/) _username to access the container registry_

_DOCKER\_PASSWORD - Your_ [_Dockerhub_](https://hub.docker.com/) _password to access the container registry_
...
_KNOWN\_HOSTS - You will need the lines in your local computer's **.ssh/known\_hosts** file relevant to the environments that the SSH Key uses. This will have been generated using a test SSH connection using your key_&#x20;
