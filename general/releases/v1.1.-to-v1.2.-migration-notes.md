...
{% hint style="warning" %}
**Are you just upgrading from v1.2\* to a newer hotfix of v1.2\*?**  If so, this process is easy. You can just follow step 2 and step 3 then use the new version in your servernot  deploy scripts. Ansible changes and data migrations are required on any hotfix release.  **You do not need to follow the instructions in step 4.  No Ansible scripts or data migrations need to run again on your servers.** **THESE NOTES ARE FOR USERS MIGRATING FROM v1.1.\* to v1.2.\* ONLY!!!!**
{% endhint %}
...
8\. When the migrations are completed, you will see the word "Done".  You can login to OpenCRVS v1.2.\* and test that all your data has migrated successfully.  Look at a registration that had attached supporting documents and you will notice that the documents are now served from Minio.
