...
v1.1.0 additionally includes the following improvements which are breaking changes.  Automated  scripts will migrate your data.

* The country configuration now loads the JWT public key from core from a new endpoint in the auth microservice.  This improves our security processes as we can now rotate the public key without taking the stack down.  It also allows development teams to start the country configuration server with yarn dev, rather than previously the requirement to run yarn dev&#x20;
* Record audit
